...
inputCheckIn.addEventListener("change", () => {
  ...
  if (
    valueDateCheckIn.slice(8, 10) * 1 === lastDayInMonth.getDate() &&
    valueDateCheckIn.slice(5, 7) * 1 !== 12
  ) {
    month =
      valueDateCheckIn.slice(5, 7) * 1 < 10
        ? `0${valueDateCheckIn.slice(5, 7) * 1 + 1}`
        : valueDateCheckIn.slice(5, 7) * 1 + 1;
    valueDateCheckOut = `${valueDateCheckIn.slice(0, 4)}-${month}-0${1}`;
  } else if (
    valueDateCheckIn.slice(5, 7) * 1 === 12 &&
    valueDateCheckIn.slice(8, 10) * 1 === 31
  ) {
    valueDateCheckOut = `${valueDateCheckIn.slice(0, 4) * 1 + 1}-0${1}-0${1}`;
  } else {
    days =
      valueDateCheckIn.slice(8, 10) * 1 < 10
        ? `0${valueDateCheckIn.slice(8, 10) * 1 + 1}`
        : valueDateCheckIn.slice(8, 10) * 1 + 1;
